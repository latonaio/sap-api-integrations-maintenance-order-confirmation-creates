...
import (
	sap_api_caller "sap-api-integrations-maintenance-order-confirmation-creates/SAP_API_Caller"
...
func main() {
	...
	inputSDC := fr.ReadSDC("./Inputs/SDC_Maintenance_Order_Confirmation_Header_sample.json")
	accepter := getAccepter(inputSDC)
	header := inputSDC.ConvertToHeader()

	caller.AsyncPostMaintenanceOrderConfirmation(
		header,
		accepter,
	)
}
