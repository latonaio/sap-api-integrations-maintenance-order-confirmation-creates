...
import (
	...
	sap_api_output_formatter "sap-api-integrations-maintenance-order-confirmation-creates/SAP_API_Output_Formatter"
	"strings"
	"sync"
...
func (c *SAPAPICaller) Header(header *requests.Header) {
	outputDataHeader, err := c.callMaintenanceOrderConfirmationSrvAPIRequirementHeader("MaintOrderConfirmation", header)
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) callMaintenanceOrderConfirmationSrvAPIRequirementHeader(api string, header *requests.Header) (*sap_api_output_formatter.Header, error) {
